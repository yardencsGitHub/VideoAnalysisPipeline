...
desktop_results_folder = '/Users/yardenc/Documents/Experiments/Imaging/CanaryData/lrb853_15/ManualROIs/SingleROIalignments';
%% Single bird
...
for i = 1:numel(keys)
    ...
    datenums = [datenums; datenum(char(join(tokens(3:5),'_')))];
end
datenums = unique(datenums);
[locs,indx] = sort(ord);
...
%for dayn = 1:numel(datenums)
    %Day = datestr(datenums(dayn),'yyyy_mm_dd'); %'2017_03_20';
    Day = '2017_03_19';
    if ~exist(fullfile(desktop_results_folder,Day),'dir')
        mkdir(fullfile(desktop_results_folder,Day));
    end
    ...
    for sylcnt = 1:numel(syls) %[templates.wavs.segType]
        sylnum = syls(sylcnt);
         %%
    hits = [];
    durations = [];
...
%    maxs = max(s);
    ...
    hits = hits(dur_idx,:);
        for ROIs = 1:size(dff,1) %18; %[5 6 7]; %[3 9 13];%
        h=figure('Visible','off','Position',[77          91        1840         600]);
            for cond = 1:3
                subplot(1,3,cond);
                warp = conds(cond,1);
                locktoonset = conds(cond,2);
            ...
            figname = [bird_name ' Day ' Day ' syl #' num2str(sylnum) ' ManualROI #' num2str(ROIs)  '.png']; 
            saveas(h,fullfile(desktop_results_folder,Day,figname));
            hgclose(h);
        end
    end
%end
